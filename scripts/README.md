...
- `validate_site.py` checks top-level HTML integrity, CSP/transport policy, canonical-data publication guards, local references and the sitemap/index publication contract. It also prevents the repository-level `robots.txt` from regressing to a blanket `Disallow: /`, while documentation separately records that this file is not authoritative on the current `/PixelWeb/` project-site URL.
