...
`pixel-theme-bootstrap.js` provides synchronous early theme resolution on the canonical pages that currently include it: it resolves stored/System mode, sets `data-theme-mode`, `data-theme-effective`, `color-scheme` and the browser `theme-color` before deferred application runtime begins. `data/network.js` retains a defensive fallback for pages that do not include the bootstrap. Whether any remaining fallback-only page produces a perceptible opposite-theme first-paint flash is still a browser-verification question; source inspection alone does not establish that visual bug.
